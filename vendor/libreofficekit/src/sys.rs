...
fn lok_init(install_path: &Path) -> Result<*mut LibreOfficeKit, OfficeError> {
    ...
    let lok = if let Some(lok_hook) = container.libreofficekit_hook {
        lok_hook(install_path.as_ptr())
    } else if let Some(lok_hook) = container.libreofficekit_hook_2 {
        lok_hook(install_path.as_ptr(), null())
    } else {
        return Err(OfficeError::MissingLibraryHook);
    };

    Ok(lok)
}

fn lok_init_with_profile(
    install_path: &Path,
    user_profile_url: &str,
) -> Result<*mut LibreOfficeKit, OfficeError> {
    // Try initialize the container (If not already initialized)
    let container = LOK_CONTAINER.get_or_try_init(|| lok_open(install_path))?;

    let install_path = install_path.to_str().ok_or(OfficeError::InvalidPath)?;
    let install_path = CString::new(install_path)?;
    let user_profile_url = CString::new(user_profile_url)?;

    let lok_hook = container
        .libreofficekit_hook_2
        .ok_or(OfficeError::MissingProfileHook)?;

    let lok = lok_hook(install_path.as_ptr(), user_profile_url.as_ptr());
...
impl OfficeRaw {
    ...
    /// Initializes a new instance of LOK with a dedicated user profile URL
    pub unsafe fn init_with_profile(
        install_path: &Path,
        user_profile_url: &str,
    ) -> Result<Self, OfficeError> {
        let lok = lok_init_with_profile(install_path, user_profile_url)?;
